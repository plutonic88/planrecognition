...
public class PlanrecognitionExp {
	...
	public static void doFixedPolicyWithDefenseExp1(boolean withdefense, int chosenattacker, int chosenpolicy, boolean minentropy, 
		...
		testSolver(net, exploits, honeypots, chosenattacker, goals, nattackers, attackers);
	...
	public static void doFixedPolicyWithDefenseMILP(boolean withdefense, int chosenattacker, int chosenpolicy, boolean minentropy, 
		...
		//Network.generateNetwork(net, nstep, nodeamin, nodeamax ,ngoal, minedge, density, nexploits, exploits, stagesectornodes);
		
		
		Network.generateNetworkWithSector(net, nstep, nodeamin, nodeamax ,ngoal, minedge, density, nexploits, exploits, stagesectornodes, stagenodes, nattackers);
		...
		Network.addExploits(net, exploits, nnodes, stepsperstage, nstep, nattackers, overlappingexploitspercentage, exploitsector, exploitdensity);
	...
	private static List<ArrayList<Integer>>[] constructSectorNodes(ArrayList<List<Integer>> stagenodes, int nattacker, int stagelimit) {
		
		int stage = 0;
		
		List<ArrayList<Integer>>[] stagesectornodes = new List[stagelimit];
		
		
		for(List<Integer> step: stagenodes)
		{
			
			stagesectornodes[stage] = new ArrayList<>();
			...
			for(Integer nodeid: step)
			{
				...
				sectornodes.get(sector).add(nodeid);
			...
			stagesectornodes[stage] = sectornodes;
		...
		return stagesectornodes;

		
	}


	private static void testSolver(HashMap<Integer,Node> net, HashMap<Integer,Exploits> exploits, 
			HashMap<Integer,Node> honeypots, int chosenattacker, int[] goals2, int nattackers, HashMap<Integer, Attacker> attackers) throws Exception {
		...
		int [][][][] w = null;// build3DCostMatrixForAttacker(net, exploits, attackers);
		...
		int[][][][][] hpdeploymentcost = new int[totalconf][nattackers][n+hplimit][n+hplimit][e];
		...
		//buildCostVar(hpdeploymentcost, net, exploits, nattackers, e, w, slotids, hpids, honeypots, placestoallocatehp, hpdeploylimit, freehps, hplimit, attackers, nattackers);
		...
		while(!fringequeue.isEmpty())
		{
			...
			else
			{
				...
				for(int j=0; j<confcost[node.id].length; j++)
				{
					...
					{
						if(confcost[at][node.id][j][k] != 100 && confcost[at][node.id][j][k] != 0)
						{
							Node tmp = new Node(j);
							tmp.currentcost = node.currentcost + confcost[at][node.id][j][k] ;
		...
		for(int c=0; c<hpdeploymentcost.length; c++)
		{
			...
			for(int i=0; i<hpdeploymentcost[c].length; i++)
			{
				for(int j=0; j<hpdeploymentcost[c][i].length; j++)
				{
					...
					{
						if(hpdeploymentcost[c][at][i][j][k] == 0)
						{
	...
	public static void buildCostVar(int[][][][][] hpdeploymentcost, HashMap<Integer, Node> net, HashMap<Integer, Exploits> exploits,
			...
			HashMap<Integer,Node> honeypots, HashMap<Integer,int[]> placestoallocatehp, int hpdeploylimit, 
			ArrayList<Integer> freehps, int hplimit, HashMap<Integer,Attacker> attackers, int nattackers, HashMap<Integer,Integer> attackermap, HashMap<Integer,Integer> attackermapback) {
		...
		for(Integer slt: slotids.keySet())
		{
			...
			for(Integer hid: hpids.keySet())
			{
				...
				{
					int atindx = attackermap.get(at);
					
					for(int i=0; i<w[atindx].length; i++)
					{
						for(int j=0; j<w[atindx][i].length; j++)
						{
							for(int k=0; k<w[atindx][i][j].length; k++)
							{
								hpdeploymentcost[confcount][atindx][i][j][k] = w[atindx][i][j][k];
							}
				...
				int start = hpdeploymentcost[confcount][0].length- hplimit;
				
				for(int at: attackers.keySet())
				{
					int atindx = attackermap.get(at);

					for(int i=start; i<hpdeploymentcost[confcount][atindx].length; i++)
					{
						for(int j=0; j<hpdeploymentcost[confcount][atindx][i].length; j++)
						{
							for(int k=0; k<e; k++)
							{
								hpdeploymentcost[confcount][atindx][i][j][k] = 100;
								hpdeploymentcost[confcount][atindx][j][i][k] = 100;
							}
				...
				for(int index: slots.keySet())
				{
					...
					for(int at: attackers.keySet())
					{
						...
						for(Integer exid: hpnode.exploits.values())
						{
							...
							if(hpdeploymentcost[confcount][atindx][n1][hd][exid] == 100 && attackers.get(at).exploits.containsValue(ex.id))
							{
								hpdeploymentcost[confcount][atindx][n1][hd][exid] += (ex.cost-100);
							}
					...
					for(int at: attackers.keySet())
					{
						...
						for(Integer exid: n2node.exploits.values())
						{
							...
							if(hpdeploymentcost[confcount][atindx][hd][n2][exid] == 100 && attackers.get(at).exploits.containsValue(ex.id))
							{
								hpdeploymentcost[confcount][atindx][hd][n2][exid] += (ex.cost-100);
							}
	...
	public static int[][][][] build3DCostMatrixForAttacker(HashMap<Integer, Node> net,
			HashMap<Integer, Exploits> exploits, HashMap<Integer,Attacker> attackers, HashMap<Integer,Integer> atmap, HashMap<Integer,Integer> atmapback) {


		int[][][][] w = new int[attackers.size()][net.size()][net.size()][exploits.size()];

		
		
		for(int at=0; at<attackers.size(); at++)
		{

			for(int i=0; i<w[at].length; i++)
			{
				for(int j=0; j<w[at][i].length; j++)
				{
					for(int k=0; k<w[at][i][j].length; k++)
					{
						Node a = net.get(i);
						Node b = net.get(j);
						 ...
						 */
						if(a.nei.containsValue(b.id) && b.exploits.containsValue(k) && attackers.get(atmapback.get(at)).exploits.containsValue(k))
						{
	...
	public static int[][][][] build3DCostMatrixWMap(HashMap<Integer, Node> net,
			HashMap<Integer, Exploits> exploits, HashMap<Integer,Integer> netmap, HashMap<Integer,Integer> netmapback, HashMap<Integer,Attacker> attackers, int nattackers, HashMap<Integer,Integer> attackermap, HashMap<Integer,Integer> attackermapback) {


		int[][][][] w = new int[nattackers][net.size()][net.size()][exploits.size()];
			...
			for(int i=0; i<w[atindx].length; i++)
			{
				for(int j=0; j<w[atindx][i].length; j++)
				{
					for(int k=0; k<w[atindx][i][j].length; k++)
					{
						 ...
						 * right now we are allowing every exploits
						 */
						if(a.nei.containsValue(b.id) && b.exploits.containsValue(k) && attackers.get(at).exploits.containsValue(k))
						{

							Exploits ex2 = exploits.get(k);
							w[atindx][i][j][k] = ex2.cost;
